...
export async function analyzeItemImage(
  ...
  // Try OpenAI first, fallback to OpenRouter if quota exceeded or error
  let aiClient = openai;
  let model = "gpt-5";
...
Provide a JSON response with the following structure:
{
  ...
  "notes": "detailed analysis summary"
    ...
    // If OpenAI fails and we haven't tried OpenRouter yet, try OpenRouter
    if (aiClient === openai && openrouter) {
      console.log(`OpenAI failed (${error.message}), falling back to OpenRouter...`);
      try {
...
Provide a JSON response with the following structure:
{
  ...
  "notes": "detailed analysis summary"
        ...
        return fallbackResult as AIAppraisalResult;
      } catch (fallbackError: any) {
        throw new Error(`AI appraisal failed with both OpenAI and OpenRouter: ${fallbackError.message}`);
      }
    }
...
export async function generateHealthReport(
  ...
  try {
    ...
    if (!aiClient && openrouter) {
      aiClient = openrouter;
      model = "anthropic/claude-3.5-sonnet";
      console.log("Using OpenRouter for health report");
    }
...
Provide a JSON response with the following structure:
{
  ...
  "estimatedLifeRemaining": "estimated remaining operational life (e.g., '3 years', '1000 hours')"
    ...
    // If OpenAI fails and we haven't tried OpenRouter yet, try OpenRouter
    if (aiClient === openai && openrouter) {
      console.log(`OpenAI failed (${error.message}), falling back to OpenRouter for health report...`);
      try {
        const fallbackResponse = await openrouter.chat.completions.create({
          model: "anthropic/claude-3.5-sonnet",
          messages: [
...
Provide a JSON response with the following structure:
{
  ...
  "estimatedLifeRemaining": "estimated remaining operational life (e.g., '3 years', '1000 hours')"
        ...
        const fallbackResult = JSON.parse(fallbackResponse.choices[0].message.content || '{}');
        return fallbackResult as AIHealthReportResult;
      } catch (fallbackError: any) {
        throw new Error(`AI health report failed with both OpenAI and OpenRouter: ${fallbackError.message}`);
      }
    }
